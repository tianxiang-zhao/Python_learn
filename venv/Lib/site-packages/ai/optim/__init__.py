...
class RecAdamGC(Optimizer):
    """ Implementation of RecAdamGC optimizer, a variant of Adam optimizer.
...
class AdamGC(Optimizer):
    r"""Implements AdamGC algorithm with Gradient Centralization.
...
class AdamWGC(Optimizer):
    r"""Implements AdamWGC algorithm with Gradient Centralization.
    It has been proposed in `Adam: A Method for Stochastic Optimization`_.
    Arguments:
        params (iterable): iterable of parameters to optimize or dicts defining
            parameter groups
...
class RangerGC(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, k=5, alpha=0.5, n_sma_threshhold=5, betas=(0.95,0.999), 
        ...
        defaults = dict(lr=lr, k=k, alpha=alpha, step_counter=0, betas=betas, 
                        ...
                        amsgrad=amsgrad, correct_bias=correct_bias)
        super().__init__(params, defaults)
